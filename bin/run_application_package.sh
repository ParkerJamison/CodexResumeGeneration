...
# Renderer/orchestrator only. The Codex skill is responsible for drafting the
# job context, match matrix, tailored resume XML, and cover letter TXT before
# this script runs.
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
VENV_PY="$ROOT/.venv/bin/python"
GENERATOR="$ROOT/src/generateApplicationPackage.py"
...
if [[ ! -f "$GENERATOR" ]]; then
  echo "Missing application package generator: $GENERATOR" >&2
  echo "Expected CLI:" >&2
  echo "  bin/run_application_package.sh job_context.json resume.xml cover_letter.txt [options]" >&2
  exit 1
fi
