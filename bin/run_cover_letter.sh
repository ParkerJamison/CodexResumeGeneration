...
GENERATOR="$ROOT/src/generateCoverLetter.py"
...
if [[ ! -f "$GENERATOR" ]]; then
  echo "Missing cover letter generator: $GENERATOR" >&2
  echo "Expected CLI:" >&2
  echo "  bin/run_cover_letter.sh input.txt output.pdf [--html-output output.html]" >&2
  exit 1
fi
